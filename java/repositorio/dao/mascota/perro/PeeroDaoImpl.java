...
public class PeeroDaoImpl implements IDaoPerro{
    ...
    @Override
    public Integer actualizarPerro(Perro perro) {
        String sqlActualizarPerro= "UPDATE mascotas SET nombre = ?, edad = ?, raza = ?, etc. WHERE codigo = ?";
        
        try{   
           conexionDB= new ConexionDB();
           ResultSet rs=conexionDB.ejecutarConsultaSql(sqlActualizarPerro);
           if(rs.next()){
               Perro p= new Perro(rs.getString("raza"),rs.getString("nombre"), rs.getInt("edad"), rs.getFloat("peso"), rs.getString("sexo").charAt(0));
               p.setCodigoPerro(rs.getString("codigo"));
               return -1;
